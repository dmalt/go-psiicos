...
B = A(occ>0);
DipInd = [(mod(B,Nsites))', ((B - mod(B,Nsites)) / Nsites+ 1)'];
[Npairs, dummy] = size(DipInd);
adjMat = zeros(Npairs,Npairs);
Dpair = 0.0129; % Pairwise clustering distance threshold
clustSize = 10; % Clulstersize threshold 
for p1 = 1:Npairs
    for p2 = p1:Npairs
        if norm(R(DipInd(p1,1),:) - R(DipInd(p2,1),:)) < Dpair && norm(R(DipInd(p1,2),:) - R(DipInd(p2,2),:)) < Dpair
            adjMat(p1,p2) = 1;
            adjMat(p2,p1) = 1;
        end
    end
