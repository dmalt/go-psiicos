...
% -------------------------------------------------------------------------- %
    ...
    M_full_complex = ps.ProjectFromSlComplete(CT, G_small, 350);
    ...
    % G_small = 7 * G_small / norm(G_small);
    ...
    % epsilon = 0.2;     % Dual gap threshold
...
% ------------------------------------------------------------------------------- %
 ...
 %  ------------------------------------------------------------------------------ %
        ...
        for i = 1:100
            ...
            eta = dual_gap_big([real(M), imag(M)], G_small, [real(X_a), imag(X_a)],...
                               lambda, sizeA, [real(Res), imag(Res)]);
            fprintf('eta = %f, ', eta );

            A_next = sort(union(A_reduced, A_penalized));
            isthesame = isempty(setxor(A, A_next));
            disp('Saving A_reduced')
            X_a_output = X_a_reduced * Mv(:, 1:ncomp)';
            save('../output/A_reduced.mat',  'A_reduced', 'X_a_output', '-v7.3')
            if isthesame | eta < epsilon
...
% ------------------------------------------------------------------------------------------ %
        ...
        w_ = w_(:); X_next_active = diag(w_) * X_a_reduced;
        % for s = 1:Nsrc_pairs
        ...
        [~, nonzero_idx_next] = intersect(suppUnion, suppX_next);
        [~, nonzero_idx_prev] = intersect(suppUnion, suppX_prev);
    ...
    Aidx = A_reduced;
    save (strcat('../output/Output_', Subj,'_', lambda_str,'_', date, '.mat'),...
          'Aidx','X', 'N1', 'N2', 'lambda', 'CT', 'CT2','G_small', 'epsilon',...
          'tau', 'ncomp', 'ActSetChunk', 'exec_time', 'i', 'k');
end
